...
def validate_url(url):
	try:
		resp = requests.get(url,headers={"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.0.0 Safari/537.36"},timeout=3)
...
def v_inputs(request):
    if request.headers.get("Host") == 'vscanner.me':
        ...
        response['Content-Disposition'] = f"attachment; filename=vulnerable_inputs.json"
        opened.close()
        return response
    else:
        context = {"code":[4,0,4],"error":"Not Found"}
